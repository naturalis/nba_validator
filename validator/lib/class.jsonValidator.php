<?php
	...
	class JsonValidator {
		...
		private $write_validated_output=true;
		...
		public function __construct( $p )
		{
	        ...
	        // $this->sqlite_path = sprintf($this->sqlite_path,$this->job_id);
		...
		public function setSourceSystemDefaults( $p )
		{
			...
			*/
			...
			{
				$this->logClass->info(sprintf('ERROR: %s',$e->getMessage()), true);
				exit(1);
			}
		}
		...
		public function setWriteValidatedOutput($state)
		{
			if (is_bool($state))
			{
				$this->write_validated_output = $state;
			}
		}
		...
		public function run()
		{
			...
			foreach ($this->file_list as $key => $file_item)
			{
				...
				$filename_for_reports = 
					is_null($file_item["original_name"]) ? basename($file_item["file"]) : basename($file_item["original_name"]);
	    ...
	    public function getSettingsOverview()
	    {
	    	...
	    	$d += [
				...
				'write_validated_output' => $this->write_validated_output,
	    ...
	    public function getValidationOverview()
	    {
	    	return [
				...
				'write_validated_output' => $this->write_validated_output ? 'y' : 'n',
				'outfiles' => [
					...
					'ids' => $this->export_ids && isset($this->output_files['ids']) ? $this->output_files['ids'] : 'n/a',
					'non_unique_ids' => isset($this->output_files['non_unique_ids']) ? $this->output_files['non_unique_ids'] : 'n/a',
				]
			];
	    }
		...
		private function _mergeJsonSchemas()
		{
			...
			$tmpSchema = tempnam(sys_get_temp_dir(), "validator");
		...
		{
			$this->logClass->debug("validating");
		...
		{
			if (!$this->write_validated_output)
			{
				return;
			}

			$this->logClass->info("writing valid");
		...
		{
			$this->logClass->debug("writing errors");
	    ...
	    private function _exportIds()
	    {
			...
			$stmts["multiple"] = "select doc_id, count(*) as c from all_ids group by doc_id having count(*) > 1";

			if ($this->test_run) 
			{
				$this->logClass->info("test run: exporting only non-unqiue id's (if any)");
			}
			else
			{
				$this->logClass->info("exporting id's");
				$stmts["all"] = "select doc_id from all_ids";
				$stmts["unique"] = "select doc_id from unique_ids";
			}
			...
			foreach ($stmts as $key=>$stmt)
			{
				...
				{
					$this->logClass->info(sprintf("found no '%s' id's to write",$key));
				}
		...
		private function _reportNonUniqueIds()
		{
			...
			$this->logClass->info(sprintf("wrote a sample of non-unqiue ID's to %s",$save_file));
		}

	  	private function _initializeSQLite()
	  	{
	  		$this->logClass->debug("initializing SQLite");
		...
		private function _setTestRunOverrides()
		{
			...
			$this->setAllowDoubleIds(true);
			$this->setLoadErrorThreshold(0);
			$this->setExportIds(true);
			$this->setWriteValidatedOutput(false);
			$this->setWriteBrokenAndInvalid(false);

			$this->logClass->info(sprintf("test run override: allow double ids => %s",
				$this->allow_double_ids ? "y" : "n"));
			$this->logClass->info(sprintf("test run override: load error threshold => %s",
				$this->load_error_threshold));
			$this->logClass->info(sprintf("test run override: export ids => %s (non-unique only)",
				$this->export_ids ? "y" : "n"));
			$this->logClass->info(sprintf("test run override: write validated output => %s",
				$this->write_validated_output ? "y" : "n"));
			$this->logClass->info(sprintf("test run override: write broken and invalid docs => %s",
				$this->write_broken_and_invalid ? "y" : "n"));
		}
	}
