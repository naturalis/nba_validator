<?php
	...
	class inputPrepare {
		...
		private function _getProcessableDirs()
		{
			...
			{
				$this->logClass->info(sprintf("scanning directory %s",$dir));
		...
		private function _unpackArchives()
		{
			...
			{
				$this->logClass->info(sprintf("processing directory %s",$dir));
				...
				foreach($files as $file)
				{
					...
					if ($ext=="gz")
					{
						...
						{
							$this->logClass->info(sprintf("unpacking and deleting %s",$file));
						...
						{
							$this->logClass->info(sprintf("unpacking %s",$file));
			...
			foreach ($this->processable_dirs as $dir)
			{
				...
				foreach($files as $file)
				{
					...
					if ($ext=="json")
					{
						...
						{
							// json --> jsonl
							$this->logClass->info(sprintf("changing %s to %s",$file,"jsonl"));
						...
						else
						{
							...
							{
								$this->name_changes[]=$b;
								$this->logClass->info(sprintf("rewrote %s to %s",$file, "jsonl"));
							}
						}
					} 
					else
